...
const CreateIdentityPanel: FC<Props> = ({ onSuccess }) => {
  ...
  const [selectedIdentityType, setSelectedIdentityType] =
    useState<IdentityType>(IDENTITY_TYPE.TLS);
  ...
  const handleIdentityTypeChange = (identityType: IdentityType) => {
  ...
  const isTypeSelectionStep = currentStep === 1;
  const goToTypeSelectionStep = () => {
    formik.setTouched({}, false);
    setCurrentStep(1);
  };

  const headerTitle = isTypeSelectionStep ? (
    "Choose an identity type"
  ) : (
    <BackLink
      linkText="Choose type"
      title={IDENTITY_TYPE_HELP_TEXT[selectedIdentityType].title}
  ...
  return (
    <SidePanel loading={isLoading} hasError={Boolean(error)}>
      <SidePanel.Header>
        <SidePanel.HeaderTitle key={isTypeSelectionStep ? "start" : "details"}>
          {headerTitle}
          ...
          >
            <div className="choose-identity-type">
              <FormLink
                ...
                subTextBelowTitle
                onClick={() => {
                  handleIdentityTypeChange(IDENTITY_TYPE.TLS);
                  setCurrentStep(2);
                }}
              />
              <FormLink
                ...
                subTextBelowTitle
                onClick={() => {
                  handleIdentityTypeChange(IDENTITY_TYPE.BEARER_CLIENT);
                  setCurrentStep(2);
                }}
              />
              <FormLink
                ...
                subTextBelowTitle
                onClick={() => {
                  handleIdentityTypeChange(IDENTITY_TYPE.BEARER_DEVLXD);
                  setCurrentStep(2);
                }}
              />
            </div>
