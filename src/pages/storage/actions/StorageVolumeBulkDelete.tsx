...
const StorageVolumeBulkDelete: FC<Props> = ({ volumes, onStart, onFinish }) => {
  ...
  const deleteableVolumes = volumes.filter((volume) => canDeleteVolume(volume));
  const totalCount = volumes.length;
  const deleteCount = deleteableVolumes.length;
  ...
  const handleDelete = () => {
    setLoading(true);
    onStart();
    const successMessage = `${deleteableVolumes.length} ${pluralize("volume", deleteableVolumes.length)} successfully deleted`;

    deleteStorageVolumeBulk(deleteableVolumes, projectName)
      .then((results) => {
  ...
  return (
    <BulkDeleteButton
      entities={volumes}
      deletableEntities={deleteableVolumes}
      entityType="volume"
