  ...
  const filteredKeys =
    keys?.filter((key) => {
      if (query) {
        ...
        if (!key.description.toLowerCase().includes(query.toLowerCase())) {
          return false;
        }
        if (!key["access-key"].toLowerCase().includes(query.toLowerCase())) {
          return false;
        }
        if (!key["secret-key"].toLowerCase().includes(query.toLowerCase())) {
          return false;
        }
      }
      return true;
