...
const Settings: FC = () => {
  ...
  const addUserDefinedConfig = (index: number) => {
    ...
    const mutation = updateSettings({ [`user.${key}`]: value });
    mutation
      .then(() => {
        toastNotify.success(<>{`Setting user.${key}: ${value} added`}</>);
      })
      .catch((e) => {
        notify.failure(`Setting user.${key}: ${value} add failed`, e, key);
      })
      .finally(() => {
  ...
  userDefinedConfigs.forEach((_, index) => {
    rows.push({
      ...
      columns: [
        ...
        },
        {
          content: (
            <>
              <Input
                aria-label={`new-user-defined-key-${index}`}
                id={`new-user-defined-key-${index}`}
                placeholder="User key"
                type="text"
                value={userDefinedConfigs[index].key}
                error={
              ...
              />
              {index == userDefinedConfigs.length - 1 && (
                <p className="p-text--small u-text--muted u-no-margin--bottom">
                  <ConfigFieldDescription
                    description={
                      "Key will be saved as user.`{your-key}`. Enter only the part after user."
                    }
                  />
                </p>
              )}
        ...
        },
        {
          content: (
            <>
              <Input
                aria-label={`new-user-defined-value-${index}`}
