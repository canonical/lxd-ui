...
test("opening /ui redirects to the configured login project", async ({
  ...
  await page.getByTitle("Edit login project").click();
  await page
  ...
  await expect(page).toHaveURL(new RegExp(`/ui/project/${project}`));
  ...
  await gotoURL(page, "/ui/");
  await expect(page).toHaveURL(new RegExp(`/ui/project/default`));
});
